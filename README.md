...
| Collaborative filtering | implicit (ALS), scipy sparse matrices |
...
Download `ratings_Electronics.csv` from [Kaggle](https://www.kaggle.com/datasets/saurav9786/amazon-product-reviews) and place it in the `data/` folder.
...
| `src/collaborative.py` | ALS collaborative filtering model |
| `src/content_based.py` | Cosine similarity content-based model |
| `src/hybrid.py` | Hybrid recommendation model |
| `data/ratings_clean.csv` | Cleaned dataset (347K ratings) |
| `notebooks/01_eda.ipynb` | EDA and data cleaning notebook |
...
1. **Raw data** (7.8M ratings) → **10-core filtering** → 347K clean ratings
2. Clean ratings feed into two parallel models — Collaborative (ALS) and Content-Based (Cosine Similarity)
3. Both models' scores are normalized and blended → **Hybrid recommendations**
...
|---|---|---|
| Collaborative | ALS, 50 factors, 20 iterations | Existing users with rating history |
| Content-Based | Cosine similarity on user-product matrix | Product-to-product similarity |
| Hybrid | 50/50 normalized weighted blend | Best overall recommendations |
| Cold Start | Content-based on manual input | Brand new users |
